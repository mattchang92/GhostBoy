...
Cartridge *Cartridge::getCartridge(string romPath)
{
	...
	if (romFileStream.is_open()) {
		...
		cout << "Rom could not be open. GB will likely hang.";
		romOpenSuccess = false;
	}
	if (romOpenSuccess) {
		...
		// Just create a NOMBC type with 0xFF filled data if there's no cartridge.
		// Kind of a hack
		returnCart = new NOMBC(romData, romSize);
	}
