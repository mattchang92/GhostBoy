...
Input::Input(bool player2) : player2(player2)
{
	P1Data = 0xFF;
	controlMode = false;
}
...
uint8_t Input::recieveData() {
	...
	if (!player2) {
		...
		if ((P1Data & 0x30) == 0x20) {
			// Right
			controlBits |= (keys[SDL_SCANCODE_KP_6] ? 0 : 1) << 0;
			// Left
			controlBits |= (keys[SDL_SCANCODE_KP_4] ? 0 : 1) << 1;
			// Up
			controlBits |= (keys[SDL_SCANCODE_KP_8] ? 0 : 1) << 2;
			// Down
			controlBits |= (keys[SDL_SCANCODE_KP_5] ? 0 : 1) << 3;
		}
		// P15
		else if ((P1Data & 0x30) == 0x10) {
			// A
			controlBits |= (keys[SDL_SCANCODE_END] ? 0 : 1) << 0;
			// B
			controlBits |= (keys[SDL_SCANCODE_DELETE] ? 0 : 1) << 1;
			// Select
			controlBits |= (keys[SDL_SCANCODE_PAGEDOWN] ? 0 : 1) << 2;
			// Start
			controlBits |= (keys[SDL_SCANCODE_PAGEUP] ? 0 : 1) << 3;
		}
		else {
			controlBits = 0xF;
		}
